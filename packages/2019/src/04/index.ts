...
function hasDupe(str: string): boolean {
  for (const dupe of dupes) {
    if (str.includes(dupe)) return true;
  }
  return false;
}

function hasIncrease(str: string): boolean {
...
export function hasDoubleDupe(str: string): boolean {
  for (const dupe of dupes) {
...
export function checkInput(iStr: string): boolean {
  if (!hasDupe(iStr)) return false;
  if (!hasIncrease(iStr)) return false;
  return true;
}
